...
def get_api_key() -> str | None:
    """Read a key from an environment variable or Streamlit secrets, never from code."""
...
def _client():
    ...
    if not api_key:
        raise AIServiceError(
            "Gemini is not configured. Add GEMINI_API_KEY to your environment or "
            "Streamlit secrets, or turn on Demo Mode for an offline demonstration."
        )
    try:
        ...
        raise AIServiceError("The Gemini library is missing. Run pip install -r requirements.txt.") from error
...
def _response_text(response) -> str:
    ...
    if not text or not text.strip():
        raise AIServiceError(
            "Gemini did not return usable text. The request may have been blocked or "
...
def _friendly_api_error(error: Exception) -> AIServiceError:
    ...
    if any(word in message for word in ("api key", "401", "403", "permission")):
        return AIServiceError("Gemini could not verify the API key. Check your key and try again.")
    if any(word in message for word in ("429", "quota", "rate limit", "resource exhausted")):
        return AIServiceError("Gemini's request limit was reached. Wait a moment and try again.")
    if any(word in message for word in ("timeout", "network", "connection")):
        return AIServiceError("A network problem prevented Gemini from responding. Check your connection and try again.")
    return AIServiceError("Gemini could not generate a response right now. Please try again later.")
